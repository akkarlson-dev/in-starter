...
5. **Web app deploy** — Deploy → New deployment → type **Web app**. Execute as **Me**, who has access **Anyone** (not "Anyone with Google account" — that setting blocks the in-app browser case in step 7 below with a silent failure, not an obvious error). Copy the deployment URL — that's their Calendar dashboard (`doGet()` in `setup.gs`).
